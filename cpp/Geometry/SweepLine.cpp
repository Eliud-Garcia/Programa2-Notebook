...
bool anySegmentIntersection(vector<Segment>& segs) {
    ...
    for (auto& ev : events) {
        cmp.x = ev.x;
        if (ev.type == 0) {  // Insertar segmento
            auto it = active.insert(ev.seg).first;
            auto it_prev = prev_it(it), it_next = next_it(it);
            if (it_prev != active.end() &&
                segmentsIntersect(it_prev->p, it_prev->q, it->p, it->q))
                return true;
            if (it_next != active.end() &&
                segmentsIntersect(it_next->p, it_next->q, it->p, it->q))
                return true;
        } else {  // Eliminar
...
int main() {
    vector<Segment> segs = {
        {{0, 0}, {4, 4}, 1}, {{1, 4}, {4, 1}, 2}, {{5, 5}, {7, 7}, 3}};
    cout << (anySegmentIntersection(segs) ? "Sí hay intersección"
                                          : "No hay intersección")
         << "\n";
